...
class XRayImageComparator:
    ...
    def load_images(self):
        ...
        if self.image1 is None or self.image2 is None:
            raise ValueError("Cannot load images, please check file paths")
        
        print(f"Image 1 size: {self.image1.shape}")
        print(f"Image 2 size: {self.image2.shape}")
        
    def align_images(self):
        if self.image1.shape != self.image2.shape:
            print("Images have different sizes, aligning...")
            ...
            print(f"Aligned size: {self.image1.shape}")
    ...
    def detect_lesions(self, threshold=30, min_area=500, max_lesions=8, merge_distance=100):
        ...
        print(f"Detected {len(self.lesion_boxes)} main lesion regions (original: {len(raw_boxes)})")
    ...
    def analyze_progression(self):
        ...
        if total_area > 10000 or avg_intensity > 80:
            progression = "Significant progression"
            severity = "Severe"
        elif total_area > 5000 or avg_intensity > 60:
            progression = "Disease progressing"
            severity = "Moderate"
        elif total_area > 1000 or avg_intensity > 40:
            progression = "Minor changes"
            severity = "Mild"
        else:
            progression = "Stable"
            severity = "Stable"
    ...
    def visualize_comparison(self, save_path=None):
        ...
        axes[0, 0].set_title('Image 1 (Early)', fontsize=14, fontweight='bold')
        axes[0, 0].axis('off')
        
        axes[0, 1].imshow(img2_rgb)
        axes[0, 1].set_title('Image 2 (Late)', fontsize=14, fontweight='bold')
        axes[0, 1].axis('off')
        
        axes[1, 0].imshow(img1_with_boxes_rgb)
        axes[1, 0].set_title('Image 1 - Lesion Markers', fontsize=14, fontweight='bold')
        axes[1, 0].axis('off')
        
        axes[1, 1].imshow(img2_with_boxes_rgb)
        axes[1, 1].set_title('Image 2 - Lesion Markers', fontsize=14, fontweight='bold')
        axes[1, 1].axis('off')
        
        info_text = f"""
        Disease Progression Report:
        ========================
        Progression: {progression_info['progression']}
        Severity: {progression_info['severity']}
        Lesion Count: {progression_info['lesion_count']}
        Total Area: {progression_info['total_area']:.0f} px^2
        Avg Intensity: {progression_info['avg_intensity']:.1f}
        """
    ...
    def create_difference_heatmap(self, save_path=None):
        ...
        axes[0].set_title('Image 1 (Early)', fontsize=14, fontweight='bold')
        axes[0].axis('off')
        
        axes[1].imshow(img2_rgb)
        axes[1].set_title('Image 2 (Late)', fontsize=14, fontweight='bold')
        axes[1].axis('off')
        
        axes[2].imshow(heatmap_rgb)
        axes[2].set_title('Difference Heatmap (Red=High Difference)', fontsize=14, fontweight='bold')
        axes[2].axis('off')
...
def main():
    ...
    comparator = XRayImageComparator(image1_path, image2_path)
    ...
    try:
        ...
        comparator.create_difference_heatmap(
            save_path=os.path.join(output_dir, "difference_heatmap.png")
        )
